...
for data in "${datasets_low_density[@]}"; do
  for leaf in "${MAX_POINTS_LEAF[@]}"; do
    for umbral in "${UMBRALES_PODA[@]}"; do
      ./build/octrees-benchmark -i "$data" -o "$FOLDER/subset" --kernels "cube,sphere" -r "1.0,2.0,3.0,5.0,6.0,7.0,8.0,10.0" -s "$N_SEARCHES" --repeats 5 -a "$ALGOS_RADIUS" --local-reorder "$LOCAL_REORDERS" --max-leaf "$leaf" --umbral-poda "$umbral"
    done
  done
done
...
for data in "${datasets_mid_density[@]}"; do
  for leaf in "${MAX_POINTS_LEAF[@]}"; do
    for umbral in "${UMBRALES_PODA[@]}"; do
      ./build/octrees-benchmark -i "$data" -o "$FOLDER/subset" --kernels "cube,sphere" -r "0.25,0.5,0.75,1.0,1.5,2.0,3.0,5.0" -s "$N_SEARCHES" --repeats 5 -a "$ALGOS_RADIUS" --local-reorder "$LOCAL_REORDERS" --max-leaf "$leaf" --umbral-poda "$umbral"
    done
  done
done
...
for data in "${datasets_high_density[@]}"; do
  for leaf in "${MAX_POINTS_LEAF[@]}"; do
    for umbral in "${UMBRALES_PODA[@]}"; do
      ./build/octrees-benchmark -i "$data" -o "$FOLDER/subset" --kernels "cube,sphere" -r "0.005,0.01,0.025,0.05,0.1,0.15,0.2,0.3" -s "$N_SEARCHES" --repeats 5 -a "$ALGOS_RADIUS" --local-reorder "$LOCAL_REORDERS" --max-leaf "$leaf" --umbral-poda "$umbral"
    done
  done
done
